...
<body>
    ...
    //$mysqli = new mysqli('localhost', 'root', '', 'pytanka_ppadol');
    $mysqli = mysqli_connect('localhost', 'root', '', 'pytanka_ppadol');
    //$mysqli = mysqli_connect('mysql.ct8.pl', 'm25367', 'zaq1@WSX', 'm25367_quiz_test');


    $mysqli->set_charset('utf8mb4');
    echo '<script type="text/javascript">console.log("połączono z bazą danych");</script>';

    // zwracamy panel admina
    if (isset($_SESSION['adminSession'])) {
        return header('location: admin/index.php');
    }
    // zwracamy panel użytkownika
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        switch ($_GET['action']) {
            case 'login':
                echo '<script type="text/javascript">console.log("post=login");</script>';
